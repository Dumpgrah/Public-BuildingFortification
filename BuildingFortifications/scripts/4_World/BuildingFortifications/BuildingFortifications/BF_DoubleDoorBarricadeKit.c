class BF_DoubleDoorBarricadeKit extends BuildingFortficationsKit
{
	override void OnPlacementComplete(Man player, vector position = "0 0 0", vector orientation = "0 0 0")
	{
		...
		if (GetGame().IsServer())
		{
			...
			BF_DoubleDoorBarricade DoubleDoorBar = BF_DoubleDoorBarricade.Cast(GetGame().CreateObjectEx("BF_DoubleDoorBarricade", GetPosition(), ECE_PLACE_ON_SURFACE));
			
			DoubleDoorBar.SetPosition(position);
			DoubleDoorBar.SetOrientation(orientation);
