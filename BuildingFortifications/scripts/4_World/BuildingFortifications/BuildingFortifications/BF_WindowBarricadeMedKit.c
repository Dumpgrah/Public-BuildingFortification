class BF_WindowBarricadeMedKit extends BuildingFortficationsKit
{
	override void Open()
	{
		...
		ReplaceItemWithNewLambdaBase lambda = new ReplaceItemWithNewLambdaBase(this, "BF_WindowBarricadeKit");
	...
	override void OnPlacementComplete(Man player, vector position = "0 0 0", vector orientation = "0 0 0")
	{
		...
		if (GetGame().IsServer())
		{
			...
			BF_WindowBarricadeMed Barricade = BF_WindowBarricadeMed.Cast(GetGame().CreateObjectEx("BF_WindowBarricadeMed", GetPosition(), ECE_PLACE_ON_SURFACE));
